...
export function GlassModeProvider({ children, config }: GlassModeProviderProps): ReactElement {
  const { appearance } = useClockSettings();

  if (appearance === "frosted") {
    return <FrostedGlassProvider config={config}>{children}</FrostedGlassProvider>;
  }
