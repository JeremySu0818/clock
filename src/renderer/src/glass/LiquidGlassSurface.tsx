...
export function LiquidGlassSurface({
  ...
  const textContrastToneRef = useRef<TextContrastTone>(
  ...
  const [textContrastTone, setTextContrastTone] = useState<TextContrastTone>(
    controlledTextContrastTone,
  );
  ...
  useEffect(() => {
    ...
    void video
      ...
      .catch((error: unknown) => {
        const message =
          error instanceof Error ? error.message : 'Video play failed';
  ...
  useLayoutEffect(() => {
    ...
    const updateBounds = (): void => {
      ...
      const sampleTarget = surface.querySelector<HTMLElement>(
        CONTRAST_SAMPLE_SELECTOR,
      );
      if (!sampleTarget) {
    ...
    const renderFrame = (): void => {
      ...
      if (
        autoTextContrast &&
        contrastFrameRef.current >= CONTRAST_SAMPLE_INTERVAL
      ) {
        ...
        try {
          ...
          const cropX =
            (windowBounds.x + currentBounds.left + sampleX - displayBounds.x) *
            ratioX;
          const cropY =
            (windowBounds.y + currentBounds.top + sampleY - displayBounds.y) *
            ratioY;
