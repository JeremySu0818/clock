...
import { SWITCH_THUMB_GLASS_CONFIG, SWITCH_TRACK_GLASS_CONFIG } from "../settings/settings-constants";
...
export function FeatureWorkspace({
  ...
  useEffect(() => {
    ...
    alarms.forEach((alarm) => {
      ...
      setRingingAlarmIds((ids) => (ids.includes(alarm.id) ? ids : [...ids, alarm.id]));
      ...
      try {
        ...
        // Audio context can be blocked on some systems; keep visual ringing state.
      }
    });
  }, [alarms, now]);
  ...
  return (
    <div className="settings-panel" aria-label={title} style={{ display: "flex", flexDirection: "column", position: "absolute", inset: 0, padding: "0 34px" }}>
      <button
        type="button"
      ...
      <section className="settings-content" aria-label={title} style={{ flex: 1, minHeight: 0, paddingLeft: 0, paddingTop: 0 }}>
        <div className="settings-tab-pane" style={{ paddingTop: 26, paddingBottom: 26 }}>
          <h2 className="settings-title" style={{ fontSize: 24, marginBottom: 8, marginTop: 0 }}>{title}</h2>
          {mode === "world-clock" ? (
            WORLD_CLOCK_ZONES.map((zone) => {
              ...
              return (
                <div key={zone.timeZone} className="settings-row">
                  <span className="settings-copy">
                    ...
                    <span className="settings-control-content" style={{ opacity: 0.6, fontSize: 12 }}>
                      {zone.note}
                    </span>
                  </span>
                  <div style={{ textAlign: "right" }}>
                    <span className="settings-title" style={{ fontSize: 24, letterSpacing: "0.02em" }}>
                      {time}
                    </span>
                    <span className="settings-control-content" style={{ opacity: 0.6, fontSize: 12 }}>
                      {dateFormatter.format(now)} · {formatOffset(zone.timeZone, now)}
                    </span>
                  </div>
                </div>
              );
            ...
            <>
              <div className="settings-row" style={{ gap: 8 }}>
                <div className="settings-menu-button" style={{ width: 110, borderRadius: 60 }}>
                ...
                <div className="settings-menu-button" style={{ flex: 1, borderRadius: 60 }}>
                  <SettingsGlass className="settings-menu-button-glass">
                    <input
                      type="text"
                      value={draftLabel}
                ...
                >
                  <SettingsGlass className="settings-menu-button-glass">
                    <span className="settings-control-content" style={{ padding: "0 4px" }}>新增</span>
                  </SettingsGlass>
                </button>
              </div>
              ...
              {alarms.map((alarm) => (
                <div key={alarm.id} className="settings-row">
                  <span className="settings-copy">
                    <span className="settings-title" style={{ fontSize: 22, letterSpacing: "0.02em" }}>
                      {alarm.time}
                    </span>
                    <span className="settings-control-content" style={{ opacity: 0.6, fontSize: 12 }}>
                      {alarm.label}
                      {ringingAlarmIds.includes(alarm.id) ? (
                        <span style={{ color: "rgb(255 212 156)", marginLeft: 8 }}>響鈴中</span>
                      ) : null}
                    </span>
                  </span>
                  <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                    <button
                      ...
                      onClick={() => {
                        ...
                        setAlarms((items) =>
                          items.map((item) =>
                            item.id === alarm.id ? { ...item, enabled: checked } : item
                          )
                        );
                      }}
                    >
                    ...
                    <button
                      ...
                      style={{ width: "auto" }}
                      onClick={() => {
                        setAlarms((items) => items.filter((item) => item.id !== alarm.id));
                        setRingingAlarmIds((ids) => ids.filter((id) => id !== alarm.id));
                      }}
                    >
                      <SettingsGlass className="settings-menu-button-glass" style={{ minHeight: "26px", padding: "0 10px" }}>
