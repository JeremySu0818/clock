...
contextBridge.exposeInMainWorld('desktopGlass', {
  getWindowMetrics: () =>
    ipcRenderer.invoke(
      GET_WINDOW_METRICS_CHANNEL,
    ) as Promise<DesktopGlassMetrics>,
  onWindowMetrics: (listener: (metrics: DesktopGlassMetrics) => void) => {
    const subscription = (
      _event: unknown,
      metrics: DesktopGlassMetrics,
    ): void => {
      listener(metrics);
    };
...
contextBridge.exposeInMainWorld('clockSettings', {
  ...
  getSettings: () =>
    ipcRenderer.invoke(GET_SETTINGS_CHANNEL) as Promise<ClockSettings>,
  getSettingsVisible: () =>
    ipcRenderer.invoke(GET_SETTINGS_VISIBILITY_CHANNEL) as Promise<boolean>,
  ...
  onSettingsVisibilityChanged: (listener: (visible: boolean) => void) => {
    ...
    return (): void => {
      ipcRenderer.removeListener(
        SETTINGS_VISIBILITY_CHANGED_CHANNEL,
        subscription,
      );
    };
  },
