...
for (const entry of fs.readdirSync(releaseDir, { withFileTypes: true })) {
  ...
  const shouldKeep = entry.isFile() && /^Clock-.*\.exe$/u.test(entry.name);

  if (!shouldKeep) {
    if (!isInsideRelease(fullPath)) {
      throw new Error(
        `Refusing to remove artifact outside release directory: ${fullPath}`,
      );
    }
...
const portableExecutables = fs
  ...
  .filter((fileName) => /^Clock-.*\.exe$/u.test(fileName));
